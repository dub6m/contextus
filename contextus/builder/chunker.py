    ...
    def _proposition_generation_prompt(self, elements: list[BoundaryElementView]) -> str:
        ...
        return "\n".join(
            [
                ...
                "A weak subject only describes how information is being presented, organized, mentioned, planned, or explained. A strong subject is the document-native object the fact is actually about: the problem, algorithm, term, symbol, assumption, bound, procedure, claim, or condition.",
                "When the source is framed through presentation language, rewrite the proposition so the document-native object carries the fact. Preserve the presentation frame only when the presentation frame itself is the fact.",
                "For title-only content, return the title itself as the proposition; do not rewrite it as a fact about the document title.",
                "For goal, plan, or presentation-roadmap sentences, name the actual algorithm/problem/assumption when the source supplies it.",
                "Preserve symbols, variables, formulas, quantities, algorithm names, and asymptotic bounds exactly when possible.",
                "Procedure steps may remain imperative when the source is an algorithm/list step.",
                "Examples:",
                "Bad: The document discusses the closest pair problem.",
                "Good: The closest-pair problem asks for a pair of points with smallest possible distance.",
                "[The weak presentation subject was removed. The problem is the document-native object carrying the fact.]",
                "Bad: The document title is Closest Pair of Points in the Plane.",
                "Good: Closest Pair of Points in the Plane.",
                "[The title is preserved as a title-only proposition instead of being converted into a document-wrapper fact.]",
                "Bad: Element of type formula on page 3.",
                "Good: Formula: Let n = |P'| = |P'_x| = |P'_y|.",
                "[The formula content is the document-native object. The extraction artifact is not the fact.]",
                "Bad: We use these assumptions later.",
                "Good: The distance-computation and membership assumptions are used to compute the algorithm running time.",
                "[The narrator was removed. The assumptions and their role carry the fact.]",
                "Bad: The goal is to present an algorithm.",
                "Good: The goal is to present an O(n log n) algorithm for the closest-pair problem.",
                "[The weak presentation frame is only kept because the source itself is a goal statement; the proposition still names the algorithm, bound, and problem.]",
                "Bad: This can be solved quickly.",
                "Good: The closest-pair problem can be solved in O(n^2) time by checking all distinct point pairs.",
                "[The vague local reference was replaced with the document-native problem name.]",
                "Bad: The sentence defines d.",
                "Good: d(pi, pj) denotes the standard Euclidean distance between pi and pj.",
                "[The sentence wrapper was removed. The symbol and its meaning carry the fact.]",
                "",
